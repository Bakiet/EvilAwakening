...
public class FPSRigidBodyWalker : MonoBehaviour {
	...
	public float InputLerpSpeed = 3.0f;//movement lerp speed that can be edited in the inspector
	...
	private CapsuleCollider leanCol;
	[HideInInspector]
	...
	public float crouchSpeedPercentage = 0.35f;//percentage to decrease movement speed while crouching
	...
	public float proneCamHeight = -0.3f;//y position of camera while prone
	...
	private bool crouchRisen;//player has risen from crouch postition
	[HideInInspector]
	...
	void Start (){
		...
		capsuleCastHeight = capsule.height * 0.375f;
...
//	}
	...
	void Update(){
		...
		if(allowProne){
			...
			if(Camera.main.transform.position.y - myTransform.position.y > (standingCamHeight * 0.95f)){
				camDampSpeed = 0.1f;
				if(!proneRisen && !prone){
					proneRisen = true;	
				}
		...
		if(Camera.main.transform.position.y - myTransform.position.y > (crouchingCamHeight * 0.95f)){
			if(!crouchRisen){
				camDampSpeed = 0.1f;
				crouchRisen = true;	
			}
		...
		if(grounded){
			if(jumping){
				...
				if (InputComponent.jumpPress
					...
					if (Camera.main.GetComponent<Animation>().IsPlaying("CameraJump")){
						//rewind animation if already playing to allow overlapping playback
						Camera.main.GetComponent<Animation>().Rewind("CameraJump");
					}
					Camera.main.GetComponent<Animation>().CrossFade("CameraJump", 0.35f,PlayMode.StopAll);
				...
				if(!capsuleTooSteep || climbing || swimming || (capsuleTooSteep && !rayTooSteep)){
					...
					if(climbing){
						if(InputComponent.moveY > 0.1f){//move player up climbable surface if pressing forward button
							...
							inputYSmoothed = 1;//to cycle bobbing effects
