...
public class WeaponBehavior : MonoBehaviour {
	...
	void Start (){
		weaponObj = transform.parent.gameObject;
	...
	void OnEnable () {
		...
		if(!unarmed){
			...
			if(Time.timeSinceLevelLoad > 2 && PlayerWeaponsComponent.switching){//don't ready weapon on level load, just when switching weapons
				...
				weaponMesh.GetComponent<Animation>()["Ready"].speed = readyAnimSpeed;
				weaponMesh.GetComponent<Animation>().CrossFade("Ready",0.35f,PlayMode.StopSameLayer);
			}
		}
		...
		if(Time.timeScale > 0 && Time.deltaTime > 0){//allow pausing by setting timescale to 0
			...
			if(!unarmed){
				...
				if(FPSPlayerComponent.hitPoints >= 1.0f){
					...
					|| FPSWalkerComponent.hideWeapon){//cancel reload if player is climbing, swimming, or holding object and weapon is lowered
				    	if(IronsightsComponent.reloading){
							...
							if(bulletsToReload != bulletsPerClip){
								...
								weaponMesh.GetComponent<Animation>()["Neutral"].speed = 1.5f;
								weaponMesh.GetComponent<Animation>().Play("Neutral", PlayMode.StopSameLayer);//play Neutral animation	
							}	
							...
							if(bulletsToReload == bulletsPerClip && reloadStartTime + reloadTime / 2 < Time.time && !sprintReloadState){
								...
								if(bulletsToReload == bulletsPerClip){
									...
									weaponMesh.GetComponent<Animation>().CrossFade("Reload", 0.35f, PlayMode.StopSameLayer);//play reloading animation
								}		
							}
						}
					}else{
					...
					if(shootStartTime + 0.75f < Time.time){	
						bulletsJustFired = 0;	
					}
				...
				|| !FPSWalkerComponent.moving){
					if(sprintAnimState){//animate weapon up
						...
						if(!PistolSprintAnim){
							...
							GetComponent<Animation>().CrossFade("RifleSprinting", 0.35f,PlayMode.StopSameLayer);
						}else{
							...
							GetComponent<Animation>().CrossFade("PistolSprinting", 0.35f,PlayMode.StopSameLayer);	
						}
					...
					if(!sprintAnimState){//animate weapon down
						...
						if(PlayerWeaponsComponent.switching && IronsightsComponent.reloading){
							weaponMesh.GetComponent<Animation>().CrossFade("Reload", 0.35f,PlayMode.StopSameLayer);
						}
					}
				}
			}
		...
		if(Time.timeScale > 0){//allow pausing by setting timescale to 0
			...
			if(!unarmed && FPSPlayerComponent.hitPoints > 0){
				...
				if(burstState){
					...
					|| IronsightsComponent.reloading
					|| PlayerWeaponsComponent.switching 
					|| GetComponent<Animation>()["RifleSprinting"].normalizedTime > 0.35f//weapon not centered after sprint
					|| ((startTime + readyTime) > Time.time)){
						burstState = false;
						burstShotsFired = 0;	
					}
				...
				if(shootStartTime + 0.4f < Time.time){
					shooting = false;	
				}
		...
		//do not allow shooting when sprinting
	   	if (canShoot){
			...
			if(meleeSwingDelay == 0){//if this is not a melee weapon
				...
				weaponMesh.GetComponent<Animation>().CrossFade("Fire", 0.35f,PlayMode.StopSameLayer);//play firing animation
				...
				if(swingSide){//determine which side to swing melee weapon
					...
					Camera.main.GetComponent<Animation>().CrossFade("CameraMeleeSwingRight", 0.35f,PlayMode.StopSameLayer);//play camera view animation
					...
					weaponMesh.GetComponent<Animation>().CrossFade("MeleeSwingRight", 0.1f,PlayMode.StopSameLayer);//play weapon swing animation
					...
					Camera.main.GetComponent<Animation>().CrossFade("CameraMeleeSwingLeft", 0.35f,PlayMode.StopSameLayer);//play camera view animation
					...
					weaponMesh.GetComponent<Animation>().CrossFade("MeleeSwingLeft",0.1f,PlayMode.StopSameLayer);//play weapon swing animation
			...
			&& !FPSWalkerComponent.hideWeapon){//dont allow reloading if gun is lowered while climbing, swimming, or holding object
				...
				if(ammo > 0){//if player has no ammo in their inventory for this weapon, do not proceed with reload
					...
					if(bulletsToReload == bulletsPerClip){
						...
						if(!PistolSprintAnim){
							if(weaponNumber == 5 || weaponNumber == 6){
								...
								Camera.main.GetComponent<Animation>().CrossFade("CameraReloadAK47", 0.35f,PlayMode.StopSameLayer);
							}else{
								...
								Camera.main.GetComponent<Animation>().CrossFade("CameraReloadMP5", 0.35f,PlayMode.StopSameLayer);
							}
						}else{
							...
							Camera.main.GetComponent<Animation>().CrossFade("CameraReloadPistol", 0.35f,PlayMode.StopSameLayer);
						}
					...
					if(bulletsToReload == bulletsPerClip){
						...
						Camera.main.GetComponent<Animation>().CrossFade("CameraReloadSingle", 0.35f,PlayMode.StopSameLayer);
